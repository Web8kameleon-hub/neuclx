...
def run(iterations=2000):
    ...
    for _ in range(iterations):
        start=time.perf_counter_ns(); result=kernel.answer("sovereign provenance cells"); samples.append(time.perf_counter_ns()-start)
    ordered=sorted(samples)
    return {"schema_version":"1.0","state":"measured","workload":"deterministic-token-overlap-v1","iterations":iterations,"result_state":result.state.value,"latency_ns":{"median":int(statistics.median(samples)),"p95":ordered[int(.95*(len(ordered)-1))],"min":min(samples),"max":max(samples)},"environment":{"python":platform.python_version(),"platform":platform.platform()}}
